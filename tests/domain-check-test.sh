...
assert_contains 'scripts/domain-check-go-benchmark.sh' "$executable_mode_block" \
  'CI checks the benchmark helper executable bit'
assert_not_contains 'scripts/update-smartdns.sh' "$executable_mode_block" \
  'standalone CI does not reference the SmartDNS updater'
assert_not_contains 'tests/smartdns-test.sh' "$executable_mode_block" \
  'standalone CI does not reference the SmartDNS offline test'
assert_not_contains '404notfound.sh' "$executable_mode_block" \
  'standalone CI does not reference the VPS toolkit installer'
assert_not_contains 'request-cloudflare-certificate.sh' "$executable_mode_block" \
  'standalone CI does not reference the certificate tool'
